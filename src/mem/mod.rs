...
#[cfg(all(target_os = "cbe", not(target_feature = "static-syscalls")))]
mod syscalls {
    extern "C" {
        pub fn cbe_memcpy_(dest: *mut u8, src: *const u8, n: u64);
        pub fn cbe_memmove_(dest: *mut u8, src: *const u8, n: u64);
        pub fn cbe_memset_(s: *mut u8, c: u8, n: u64);
        pub fn cbe_memcmp_(s1: *const u8, s2: *const u8, n: u64, result: *mut i32);
    }
}

#[cfg(all(target_os = "cbe", target_feature = "static-syscalls"))]
mod syscalls {
    pub(crate) fn cbe_memcpy_(dest: *mut u8, src: *const u8, n: u64) {
        let syscall: extern "C" fn(*mut u8, *const u8, u64) =
            unsafe { core::mem::transmute(1904002211u64) }; // murmur32 hash of "cbe_memcpy_"
        syscall(dest, src, n)
    }

    pub(crate) fn cbe_memmove_(dest: *mut u8, src: *const u8, n: u64) {
        let syscall: extern "C" fn(*mut u8, *const u8, u64) =
            unsafe { core::mem::transmute(1128493560u64) }; // murmur32 hash of "cbe_memmove_"
        syscall(dest, src, n)
    }

    pub(crate) fn cbe_memcmp_(dest: *const u8, src: *const u8, n: u64, result: *mut i32) {
        let syscall: extern "C" fn(*const u8, *const u8, u64, *mut i32) =
            unsafe { core::mem::transmute(1608310321u64) }; // murmur32 hash of "cbe_memcmp_"
        syscall(dest, src, n, result)
    }

    pub(crate) fn cbe_memset_(dest: *mut u8, c: u8, n: u64) {
        let syscall: extern "C" fn(*mut u8, u8, u64) =
            unsafe { core::mem::transmute(930151202u64) }; // murmur32 hash of "cbe_memset_"
        syscall(dest, c, n)
    }
}
...
pub unsafe extern "C" fn memmove(dest: *mut u8, src: *const u8, n: usize) -> *mut u8 {
    ...
    let nstore = n - (7 * chunks) as usize;
    if nstore > NSTORE_THRESHOLD {
        cbe_memmove_(dest, src, n as u64);
        return dest;
    }
...
pub unsafe extern "C" fn memset(s: *mut u8, c: c_int, n: usize) -> *mut u8 {
    ...
    let nstore = n - (7 * chunks) as usize;
    if nstore > NSTORE_THRESHOLD {
        cbe_memset_(s, c as u8, n as u64);
        return s;
    }
    let mut i: isize = 0;
    if chunks != 0 {
...
pub unsafe extern "C" fn memcmp(s1: *const u8, s2: *const u8, n: usize) -> i32 {
    ...
    if nstore > NSTORE_THRESHOLD {
        let mut result = 0;
        cbe_memcmp_(s1, s2, n as u64, &mut result as *mut i32);
        return result;
    }
    let mut i: isize = 0;
    if chunks != 0 {
